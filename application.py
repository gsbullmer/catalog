...
def gconnect():
    ...
    output += '<img src="%s" class="profile-pic">' % login_session['picture']
    output += '</div>'
    ...
    output += "<h6>We're sure glad you're here!</h6>"
    output += '</div>'
...
def fbconnect():
    ...
    app_id = json.loads(
        open(root + '/static/secrets/fb_client_secrets.json',
             'r').read())['web']['app_id']
...
def fuzzyTime(date_time):
    """
    Returns the amount of time that has passed in a vague way.
    Time is abstracted to years, weeks, days, hours, or minutes,
